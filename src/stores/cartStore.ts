...
export function cartItemKey(item: { variantId: string; customization?: CartCustomization }): string {
  const c = item.customization;
  return [item.variantId, c?.nombre || '', c?.numero || '', c?.parche || ''].join('|');
}

function buildLineAttributes(item: { customization?: CartCustomization }): Array<{ key: string; value: string }> {
  const c = item.customization;
  if (!c) return [];
  const attrs: Array<{ key: string; value: string }> = [];
  if (c.nombre) attrs.push({ key: 'Nombre', value: c.nombre });
  if (c.numero) attrs.push({ key: 'Número', value: c.numero });
  if (c.parche) attrs.push({ key: 'Parche', value: c.parche });
  return attrs;
}
  ...
  persist(
    (set, get) => ({
      ...
      addItem: async (item) => {
        ...
        try {
          if (!cartId) {
            ...
            if (result.success) {
              set({ items: get().items.map(i => cartItemKey(i) === key ? { ...i, quantity: newQuantity } : i) });
      ...
      updateQuantity: async (itemKey, quantity) => {
        ...
        const { items, cartId, clearCart } = get();
        const item = items.find(i => cartItemKey(i) === itemKey);
        if (!item?.lineId || !cartId) return;
        set({ isLoading: true });
        try {
          const result = await updateShopifyCartLine(cartId, item.lineId, quantity);
          if (result.success) set({ items: get().items.map(i => cartItemKey(i) === itemKey ? { ...i, quantity } : i) });
          else if (result.cartNotFound) clearCart();
      ...
      removeItem: async (itemKey) => {
        ...
        try {
          ...
          if (result.success) {
            const newItems = get().items.filter(i => cartItemKey(i) !== itemKey);
